...
\textit{Note: While originally planned as automated Node console scripts, these Functional Requirements evaluated via Manual UI-based Testing on the deployed application to better validate the end-to-end integration of the frontend, backend, and real-time WebSocket engine under realistic network conditions.}
...
\textbf{How test will be performed:} (Note: If Base Toggle UI is not implemented, confirm that both numeral systems are perpetually visible contextually). If implemented, click the base toggle switch. Confirm all score and target text updates instantly without losing numeric parity.
\end{enumerate}
